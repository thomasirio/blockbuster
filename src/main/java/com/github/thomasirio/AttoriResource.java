...
public class AttoriResource extends Controller {
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        try {
            if (parameters.size() == 0)
                attori = Attore.all(Attore.class);
            else {
                ...
                while (keysEnum.hasMoreElements()) {
                    ...
                    keys.add(key);
                    values.add(this.getValue(Attore.class, key, request));
                }

                attori = Attore.where(Attore.class, keys, values, null, null);
            }

            PrintWriter writer = response.getWriter();
            writer.println(attori.toJson());
